  ...
  Object? visitEnumDeclaration(dart_ast.EnumDeclaration node) {
    _enums.add(Enum(
      ...
      members: node.constants
          .map((dart_ast.EnumConstantDeclaration e) {
              final arg = e.arguments?.argumentList.toSource().replaceAll('(', '').replaceAll(')', '');
              final name = e.name2.lexeme;
              return MapEntry(name, arg);
            // return e.name2.lexeme;
          })
          .toList(),
      documentationComments:
          _documentationCommentsParser(node.documentationComment?.tokens),
